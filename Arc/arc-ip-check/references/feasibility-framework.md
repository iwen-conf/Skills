...
本文档定义 `arc-ip-check` 使用的可行性评估维度、评分标准与风险矩阵。
...
1. **独立评分**: 三Agent(architecture/deep/review)各自按维度独立评分
2. **证据验证**: 每个评分必须附文件路径或代码片段证据
3. **交叉反驳**: 各Agent互相挑战评分,指出过度乐观/悲观之处
4. **加权综合**: 按领域专长加权(软著deep 60%,专利architecture 60%)
