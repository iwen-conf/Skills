...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="将 Mermaid 时序图包装为 draw.io 文件")
    parser.add_argument("--input", required=True, help="Mermaid 时序图 .mmd 文件")
...
def extract_mermaid_source(content: str) -> str:
    ...
    source = "\n".join(captured).strip()
    if not source.startswith("sequenceDiagram"):
        raise RuntimeError("输入文件中未找到有效的 Mermaid 时序图源码")
    return source + "\n"
...
def main() -> int:
    ...
    print(f"✓ 已生成时序图 drawio 文件: {output_path}")
