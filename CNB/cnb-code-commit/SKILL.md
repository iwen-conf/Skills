...
任务属于 Bug、故障、回归或基于问题的优化时，必须先加载
[`docs/execution-truth.md`](../../docs/execution-truth.md) 的 evidence-first root-cause repair
门禁：先确认问题与根因，再确定同类路径和完整修复边界。未确认时不得修改生产行为或
宣称已修复；只允许为取证增加有界、行为保持的日志、探针或复现覆盖。
