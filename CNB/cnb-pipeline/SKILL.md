...
进入诊断模式时，必须先加载 [`docs/execution-truth.md`](../../docs/execution-truth.md) 的
evidence-first root-cause repair 门禁：确认失败或性能问题所在的构建、分支、提交、Stage
和日志证据，再追踪流水线边界与业务发布契约；禁止用重试、延长超时或跳过检查掩盖未确认
的根因。
