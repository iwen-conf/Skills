...
# 智能调度 Exec（需求分析 + Skill 路由 + 多 Agent 调度）

## Overview

`arc:exec` 是一个**元技能（Meta-Skill）**——它不直接完成具体任务，而是充当所有 `arc:` 技能的统一入口和智能调度层。工作流：
...
开始时明确说明：  
“我正在使用 `arc:exec`，先完成需求理解、Skill 路由与调度计划。”
用户也可直接说：“拉一个团队做这个任务。”
...
- **典型场景**：同一任务需要跨多个技能编排，或先做仓库摸底/任务拆解再推进执行（如 `arc:clarify` → `arc:decide` → `arc:build`）。
- **边界提示**：若已明确具体技能且边界清晰，直接调用目标技能而非 `arc:exec`。
...
- 可以装备多个 skills：`capabilities=["arc:decide", "frontend-ui-ux"]`
...
| 用户提到「UML」「类图」「时序图」「部署图」「组件图」 | arc:model |
| 用户提到「知识产权评估」「专利可行性」「软著可行性」「IP审查」 | arc:ip-check |
| 用户提到「技术交底书」「权利要求」「软著说明书」「申请文档写作」 | arc:ip-draft |
| 用户提到「讨论」「deliberate」「方案」「架构决策」 | arc:decide |
| 用户提到「实现」「编码」「开发」「重构」「落地」 | arc:build |
| 用户提到「测试」「E2E」「simulate」「浏览器」 | arc:e2e |
| 用户提到「修复」「triage」「bug」「失败」 | arc:fix |
| 用户提到「回归」「loop」「重测」 | arc:retest |
| 用户提到「拉一个团队」「组个团队」「团队并行」 | arc:exec |
...
4. **Agent 选择有据**
   ...
   - 架构设计、综合分析 → `dispatch_job(role="oracle", capabilities=["arc:decide"], ...)`
   - 需求歧义分析 → `dispatch_job(role="prometheus", capabilities=["arc:clarify"], ...)`
   - 计划质量审查 → `dispatch_job(role="metis", capabilities=["arc:decide"], ...)`
   - 全栈任务 → 拆分后并发分派多个 dispatch_job()

5. **记录调度决策**
   - 每次调度将决策过程写入 `.arc/exec/dispatch-log.md`。
...
3. 若产物过期或不一致，按索引中的 `refresh_skill` 触发回流更新（例如 `arc:init:update`、`arc:cartography`、`score` 模块刷新（由 `arc:release` 编排触发））。
...
3. **记录回滚命令**：
   ```markdown
   ## 回滚信息
   - **快照路径**: .arc/exec/snapshots/<timestamp>/
   - **回滚命令**: `tar -xzf .arc/exec/snapshots/<timestamp>/state.tar.gz`
   ```
...
|------|------|---------|
| 需求理解 | MCP 搜索 → 需求分析 → 澄清 | — |
| Skill 路由 | 决策树匹配 → 记录 | `.arc/exec/dispatch-log.md` |
| **执行预览** | 操作清单 → 快照 → 确认 → (dry-run 退出) | `.arc/exec/snapshots/` |
