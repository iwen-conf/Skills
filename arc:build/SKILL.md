...
- **首选触发**: The requirements or solutions have been clarified, and code changes can be submitted if output is required.
- **典型场景**: Function development, interface evolution, database migration, behavior-invariant reconstruction, document synchronization, defect repair with verification evidence.
- **边界提示**: `arc:clarify/arc:decide` is used for fuzzy requirements, and `arc:audit` is used for comprehensive evaluation.
...
| `output_dir` | string | no | Default `<project_path>/.arc/implement/<task-name>/` |
...
2. `.arc/deliberate/<task>/` The product of the next solution (if it exists)
...
Default directory: `<project_path>/.arc/implement/<task-name>/`
