...
arc:clarify 是一个轻量级技能，用于扫描项目的 CLAUDE.md 层级索引，为模糊的用户 prompt 补充项目上下文。作为需求澄清的入口，帮助用户明确问题、补充背景、生成增强后的 prompt。
