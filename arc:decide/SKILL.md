...
`arc:decide` uses a shared file system to organize multi-role debates (`architecture`, `deep`, `ui`), and then enters execution after "ambiguity clarification → solution game → plan solidification".
The core goal is not to "give answers quickly", but to output traceable, reviewable, and executable decision-making basis.
...
- **Inputs**: `task_name`, `workdir`, enhanced prompt words, upper limit of rounds (`max_rounds`).
...
4. Only enter the execution phase after the dispute has been resolved.
...
- Plan comparison requires "weighted scoring + sensitivity analysis" to avoid single-point preference.
...
- Generate BDD seed: `python3 arc:decide/scripts/generate_bdd_seed.py --consensus-report <report.md> --output <bdd-seed.yaml>`
- Scheduling long example: `arc:decide/references/schedule-task-playbook.md`
...
- **Boundary Tip**: If the requirement itself is not clear, use `arc:clarify` first; if the plan has been decided, switch to `arc:build`.
...
- **ace-tool MCP**: Required. Responsible for warehouse semantic retrieval and evidence location.
...
For detailed `schedule_task(...)` examples, see `arc:decide/references/schedule-task-playbook.md`.
...
1. Execute the implementation according to `tasks.md` (can be delegated to `deep` or forwarded to `arc:build`).
2. Log implementation evidence to `execution/implementation-log.md`.
3. Archive the changes after verification (`openspec archive <task-name>`).
