...
```bash
python3 arc:e2e/scripts/baseline_manager.py init \
...
### Run Comparison
```bash
python3 arc:e2e/scripts/baseline_manager.py compare \
  --run-dir {{REPORT_OUTPUT_DIR}}/{{RUN_ID}} \
  --baseline-dir baselines/<project> \
  --threshold 0.95 \
...
```bash
python3 arc:e2e/scripts/visual_diff.py \
...
```bash
python3 arc:e2e/scripts/baseline_manager.py update \
  --run-dir {{REPORT_OUTPUT_DIR}}/{{RUN_ID}} \
  --baseline-dir baselines/<project> \
...
```bash
python3 arc:e2e/scripts/check_artifacts.py \
  --run-dir {{REPORT_OUTPUT_DIR}}/{{RUN_ID}} \
