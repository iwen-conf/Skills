...
│
└── 全栈 / 混合 / 不确定
    └── 拆分为多个 schedule_task() 按 category 并行调度
...
graph TD
    ...
    subgraph Phase3[Phase 3: Agent 调度]
        DEEP["schedule_task(workstream=deep)<br/>后端/工程任务"]
        VIS["schedule_task(workstream=visual-engineering)<br/>前端任务"]
        ORACLE["schedule_task(specialist=oracle)<br/>架构咨询"]
    end
...
4. **Agent 选择有据**：后端→workstream:deep，前端→workstream:visual-engineering，架构→subagent:oracle，计划审查→subagent:momus
...
| 需求无法匹配任何 skill | 作为通用开发任务，使用 schedule_task(workstream="unspecified-high") |
|| 多 Agent 产出冲突 | 使用 schedule_task(specialist="oracle") 进行架构决策 |
...
2. **Agent 调用方式**：
   - 按领域: `schedule_task(workstream="<domain>", capabilities=[...], description="...", prompt="...", run_mode="background")`
   - 专业咨询: `schedule_task(specialist="oracle/prometheus/metis", capabilities=[...], ...)`
   - 代码搜索: `schedule_task(specialist="explore", run_mode="background", ...)`
   - 文档搜索: `schedule_task(specialist="librarian", run_mode="background", ...)`
