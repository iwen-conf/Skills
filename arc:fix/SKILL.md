...
4. If it fails, press `refresh_skill` reflow to trigger the update (`arc:init --mode update` / `arc:cartography` / `arc:audit` / `score` module refresh (triggered by `arc:release` orchestration))
