...
开始时明确说明：  
“我正在使用 `arc:decide`，先做多视角审议与分歧收敛，再进入执行。”
...
| Agent 角色 | 调用方式 | 用途 |
|------|---------|------|
| **oracle** | `dispatch_job(role="oracle", capabilities=["arc:decide"], ...)` | 架构分析、设计评审（只读高质量推理） |
| **deep** | `dispatch_job(lane="deep", capabilities=[...], ...)` | 深度工程分析、方案提案、代码执行 |
| **visual-engineering** | `dispatch_job(lane="visual-engineering", capabilities=["arc:decide", "frontend-ui-ux"], ...)` | 前端与DX视角、UI/UX设计、交互体验 |
| **metis** | `dispatch_job(role="metis", capabilities=["arc:clarify"], ...)` | 需求预分析、歧义检测 |
...
dispatch_job(
  ...
  capabilities: ["arc:decide", ...], // 装备相关 skill
  description: "<简短描述>",
...
- **边界提示**：需求不清先 `arc:clarify`；路线已定直接 `arc:build`。
...
| 角色 | 调用方式 | 并发支持 |
|------|---------|---------|
| oracle | `dispatch_job(role="oracle", capabilities=["arc:decide"], execution_mode="background", ...)` | 后台异步 |
| deep | `dispatch_job(lane="deep", capabilities=["arc:decide"], execution_mode="background", ...)` | 后台异步 |
| visual-engineering | `dispatch_job(lane="visual-engineering", capabilities=["arc:decide", "frontend-ui-ux"], execution_mode="background", ...)` | 后台异步 |
| 聚合/定稿 | 主进程直接处理 | — |
