...
Arc 收敛为七个 `arc:*` 软件工程生命周期 Skill。若任务涉及代码库搜索或上下文定位，优先使用 `.ai-code-index/` 本地搜索脚本；若项目根目录已有 `.lark.json`，优先从索引找项目主页、PRD、需求、架构、进度和交付记录；若没有 `.lark.json`，只有在用户提供飞书链接、明确触发或确认启用飞书时才使用 `arc:docs` 创建飞书资源；若任务涉及编排、Inbox 或跨 Agent 协作，优先使用 `aitask`。
...
| `arc:docs` | 已有 `.lark.json`、用户给飞书链接、或明确要求启用/创建/同步飞书资源 | 只做代码实现；项目未接入飞书且用户未触发 | `arc:define` / `arc:build` / `arc:audit` |
...
flowchart TD
    ...
    P -- 是 --> Q[arc:define]
    Q --> L{已有 .lark.json 或用户触发飞书?}
    L -- 是 --> M[arc:docs]
    L -- 否 --> B{需求是否清晰?}
    P -- 否 --> N{已有 .lark.json/飞书链接/飞书触发词?}
    N -- 是 --> M
    N -- 否 --> B{需求是否清晰?}
    B -- 否 --> C[arc:clarify]
    B -- 是 --> D{是否已有失败证据?}
