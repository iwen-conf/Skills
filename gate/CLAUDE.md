...
| 时间 | 操作 |
|------|------|
| 2026-03-03 | 初始版本，创建 arc:gate 子模块 |

## 模块职责

arc:gate 是 CI 门禁子模块，基于 arc:score 的量化数据执行可配置的质量门禁判定。支持多种门禁模式、阈值配置、豁免清单和 CI 集成能力。
...
通过 Claude Code 调用：`/arc:gate`

输入参数：
- `project_path` (required): 项目根目录
- `score_dir` (optional): arc:score 输出目录
...
| `score_dir` | string | 否 | arc:score 输出目录 |
...
├── violations.json        # 违规清单
├── whitelist-status.json  # 豁免状态
└── summary.md             # 执行摘要
```
...
| 依赖 | 类型 | 用途 |
|------|------|------|
| arc:score | 必须 | 提供量化评分数据 |
| Python 3.8+ | 必须 | 脚本执行环境 |
...
| `scripts/render_gate_report.py` | 报告渲染脚本 |
| `references/gate-config.yaml` | 门禁配置示例 |
