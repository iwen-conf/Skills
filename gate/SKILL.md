...
arc:release 是统一门禁模块，消费 `score/` 模块产出的量化数据执行可配置质量判定，并补齐上线前发布准备度检查（回滚、监控、值班与公告）。支持 warn/strict/strict_dangerous 三种模式，提供阈值配置、豁免清单和 CI 集成能力。
...
开始时明确说明：  
“我正在使用 `arc:release`，先加载评分与策略，再输出门禁判定。”
...
- **边界提示**：要做根因定位与系统性改进先用 `arc:audit`，`arc:release` 聚焦阻断判定与放行建议。
...
4. 若缺失/过期/哈希不一致，先回流触发 `score/` 模块刷新（由 `arc:release` 编排触发），再执行门禁
...
- `arc:release` 必须优先通过 `.arc/context-hub/index.json` 发现 score 产物
...
| 情况 | 处理 |
|------|------|
| score 产物不存在 | 先生成 score 产物后再运行 arc:release |
| 配置文件解析失败 | 使用默认配置并警告 |
| 豁免清单过期 | 忽略过期豁免 |
