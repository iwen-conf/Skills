...
- **Decision Tree**: 如果用户只是想修改项目代码，用 `arc:build`；如果用户要的是从实现中生成毕业支撑文档，用本 skill。

## Context Search

- 取证时先用目标仓库的 `.ai-code-index/search.sh "query"` 定位真实项目中的核心业务流、路由、控制器、数据库脚本、测试和配置。
- 若目标仓库没有索引入口，优先在其中创建或运行本地索引；索引缺失时再退回到小范围本地搜索。
- 表名、字段名、类名、路径和错误文本等精确匹配，可用 `rg` 补充，但不要把 `rg` 当成默认全仓扫描工具。
...
2. 建立事实优先级：先通过本地索引定位运行时代码，再看数据库脚本，再看项目内设计文档，最后才看同学论文的栏目和表格样式。
3. 抽取证据矩阵：
   - 角色、登录注册、权限与会话；
   - 核心业务流和对应控制器/页面；
   - 物理表与字段；
