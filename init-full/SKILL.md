...
写入 <output_dir>/agents/designer/analysis.md。"
)
```
...
**oracle 反驳 deep + designer**（用 `Task(subagent_type="oracle", session_id="<复用>", ...)`）:
- 读取 `agents/deep/analysis.md` 和 `agents/designer/analysis.md`
- 产出 `agents/oracle/critique.md`

**deep 反驳 oracle + designer**（用 `Task(subagent_type="fixer", session_id="<复用>", ...)`）:
- 读取 `agents/oracle/analysis.md` 和 `agents/designer/analysis.md`
- 从工程/构建/测试角度反驳
- 产出 `agents/deep/critique.md`

**designer 反驳 oracle + deep**（用 `Task(subagent_type="designer", session_id="<复用>", ...)`）:
- 读取 `agents/oracle/analysis.md` 和 `agents/deep/analysis.md`
- 从前端/DX/文档角度反驳
- 产出 `agents/designer/critique.md`
...
读取全部 Agent 产出（agents/oracle/、agents/deep/、agents/designer/ 下的 analysis.md + critique.md），对每个待生成目录：
1. 解决各方分歧（如版本号冲突，以 manifest 文件为准）
2. 合并各方贡献（架构来自 oracle、工程来自 deep、成熟度来自 designer）
