...
- **Trigger**：首次初始化、索引严重过时、或明确要求全量重建。
- **Inputs**：项目路径、扫描深度、模块深度、语言与输出目录。
- **Outputs**：层级 `CLAUDE.md`、`project-snapshot.md`、`generation-plan.md` 与指纹基线。
- **Quality Gate**：发布前必须通过 `## Quality Gates` 的只读安全与链接一致性检查。
...
开始时明确说明：  
“我正在使用 `arc:init:full`，将执行全量扫描并重建 CLAUDE 索引体系。”
...
未明确只读边界与证据链，不得执行全量重建。

## Workflow

1. 深度扫描项目拓扑、模块与技术栈信息。
2. 基于显著性评分制定 CLAUDE 生成计划。
3. 多 Agent 并发分析并生成层级索引文档。
4. 校验链接一致性后发布新基线与指纹。

## Quality Gates

- 仅允许写入 `CLAUDE.md` 与 `.arc/init/` 产物目录。
- 每个关键结论必须有源码或配置证据。
- 层级索引中的链接必须真实可达。
- 生成后必须更新指纹与快照元数据。
...
- **首选触发**：无可用基线或基线失效，需要全量重建 CLAUDE 索引。
- **典型场景**：首次初始化、大规模重构后、增量更新失败重置。
- **边界提示**：仅局部变更同步优先 `arc:init:update`。
...
* **统一 Dispatch API**: 必须。通过 `dispatch_job()` 调度 oracle/deep/visual-engineering Agent。
...
**oracle 反驳 deep + visual-engineering**（用 `dispatch_job(role="oracle", continuation_id="<复用>", ...)`）:
- 读取 `agents/deep/analysis.md` 和 `agents/visual-engineering/analysis.md`
- 产出 `agents/oracle/critique.md`

**deep 反驳 oracle + visual-engineering**（用 `dispatch_job(lane="deep", continuation_id="<复用>", ...)`）:
- 读取 `agents/oracle/analysis.md` 和 `agents/visual-engineering/analysis.md`
- 从工程/构建/测试角度反驳
- 产出 `agents/deep/critique.md`

**visual-engineering 反驳 oracle + deep**（用 `dispatch_job(lane="visual-engineering", continuation_id="<复用>", ...)`）:
