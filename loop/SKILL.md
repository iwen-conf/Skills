...
- **Inputs**：tmux 服务配置、`arc:e2e` 参数、`max_iterations`。
...
开始时明确说明：  
“我正在使用 `arc:retest`，先拉起服务并建立回归闭环，再逐轮验证。”
...
2. 启动/重启服务并落盘日志。
3. 执行 `arc:e2e` 产出本轮测试工件。
4. FAIL 则进入 `arc:fix` 修复，PASS 则收敛交付。
...
- FAIL → 必须交给 `arc:fix` 输出根因与修复证据（禁止“感觉修了”）
...
- **测试报告（arc:e2e）**：默认 `reports/<run_id>/`（可用 `scaffold_run.py --report-output-dir ...` 覆盖）。
- **账号管理（arc:e2e）**：`reports/<run_id>/accounts.jsonc`（明文账号/密码/Token；不得提交入库）。
...
# 然后用同一个 run_id 跑 arc:e2e，产出 reports/$run_id/
```
...
- FAIL：进入修复
  - 把当前 `run_dir` 作为输入，按 `arc:fix` 的流程定位与修复
