...
赋予 Agent “企业级软件评审专家”的能力。通过 oracle、fixer、designer 三个专业 Agent 各自独立按七维度评估项目，再互相反驳对方的评分和发现，最终收敛出一份可交付的诊断报告与改进路线图。
...
* **oh-my-opencode-slim Task API**: 必须。通过 Task() 调度 oracle/fixer/designer Agent。
...
| **fixer** | `Task(subagent_type="fixer", load_skills=["arc:review"], run_in_background=true, ...)` | 代码质量、DevOps 维度（工程专家） |
| **designer** | `Task(subagent_type="designer", load_skills=["arc:review"], run_in_background=true, ...)` | 业务、团队、体验维度(业务与流程分析专家) |
| **explorer** | `Task(subagent_type="explorer", load_skills=[], run_in_background=true, ...)` | 代码库模式搜索（廉价） |
...
**fixer 评估**（工程视角，侧重 code-quality/devops）:
```
Task(
  subagent_type: "fixer",
  load_skills: ["arc:review"],
  run_in_background: true,
  description: "fixer 七维度评估",
  prompt: "你是企业级软件评审专家，侧重后端架构、代码质量和安全。

读取 <output_dir>/context/project-snapshot.md 了解项目概况。
评估框架参考 <skills_dir>/review/references/dimensions.md。

对全部 7 个维度逐一评估，每个维度产出一个独立文件 <output_dir>/fixer/dim-N-<name>.md。
格式同 oracle（评分 + 加分点 + 扣分点 + 关键发现 + 改进建议）。
必须引用代码证据（file:line）。"
)
```

**designer 评估**(业务与流程视角,侧重 business/team):
```
Task(
  subagent_type: "fixer",
  load_skills: ["arc:review"],
  run_in_background: true,
  description: "designer 业务与流程七维度评估",
  prompt: "你是企业级软件评审专家,侧重业务价值、团队协作和流程规范。

读取 <output_dir>/context/project-snapshot.md 了解项目概况。
评估框架参考 <skills_dir>/review/references/dimensions.md。

对全部 7 个维度逐一评估,每个维度产出一个独立文件 <output_dir>/designer/dim-N-<name>.md。
格式同 oracle（评分 + 加分点 + 扣分点 + 关键发现 + 改进建议）。
必须引用代码证据（file:line）。"
)
```
...
**oracle 反驳 fixer + designer**(用 `Task(subagent_type="oracle", session_id="<复用>", ...)`)：
- 读取 `fixer/dim-*.md` 和 `designer/dim-*.md`
- 从架构视角反驳
- 产出 `oracle/critique.md`

**fixer 反驳 oracle + designer**(用 `Task(subagent_type="fixer", session_id="<复用>", ...)`)：
- 读取 `oracle/dim-*.md` 和 `designer/dim-*.md`
- 从工程/代码质量/安全角度反驳
- 产出 `fixer/critique.md`

**designer 反驳 oracle + fixer**(用 `Task(subagent_type="fixer", session_id="<复用>", ...)`)：
- 读取 `oracle/dim-*.md` 和 `fixer/dim-*.md`
- 从质量/UX/运维角度反驳
- 产出 `designer/critique.md`
...
1. 取各方评分的**专业加权平均**：
   - architecture/security/tech-debt: oracle 50%, fixer 25%, designer 25%
   - code-quality/devops: fixer 50%, oracle 25%, designer 25%
   - business/team: designer 50%, oracle 25%, fixer 25%
...
| 维度 | oracle | fixer | designer | 最终 | 评级 |
|------|--------|------|-------|------|------|
| 架构设计 | 7 | 8 | 7 | 7.3 | 良好 |
| 安全合规 | 6 | 5 | 6 | 5.6 | 合格 |
| ... | ... | ... | ... | ... | ... |
...
=== 阶段 2: 独立评估 ===
  ...
  ├── fixer(subagent) 7 维度... [完成]
  └── designer(subagent) 7 维度... [完成]

=== 阶段 3: 交叉反驳 ===
  ├── oracle 反驳 fixer+designer... [完成]
  ├── fixer 反驳 oracle+designer... [完成]
  └── designer 反驳 oracle+fixer... [完成]
...
| 独立评估 | 多Agent×7 维度 | `(oracle|fixer|designer)/dim-N-<name>.md` |
| 交叉反驳 | 各Agent互相反驳 | `(oracle|fixer|designer)/critique.md` |
...
| fixer | `Task(subagent_type="fixer", load_skills=["arc:review"], run_in_background=true, ...)` | 后台异步 |
| designer | `Task(subagent_type="designer", load_skills=["arc:review"], run_in_background=true, ...)` | 后台异步 |
| 主进程（聚合/报告） | 直接处理 | — |
