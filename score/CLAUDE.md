...
| 时间 | 操作 |
|------|------|
| 2026-03-03 | 初始版本，创建 score 内部模块；`arc:score` 对外入口后续收敛至 `arc:gate` |

## 模块职责

score 是量化评分内部模块，通过静态分析检测 Code Smell、分析 Bugfix 历史分级、检查架构违规，输出量化评分数据供 `arc:review` 与 `arc:gate` 消费。
...
由 `arc:gate` 在编排流程中触发（无独立 `arc score` 命令）
