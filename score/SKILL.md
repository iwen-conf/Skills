---
name: "arc:score"
description: "当需要量化质量评分、Code Smell 检测和机器可读评估产物时使用。"
---
...
- **Trigger**：需要可量化质量信号支持评审、门禁或技术债追踪。
- **Inputs**：项目路径、检测焦点、严重度阈值与语言范围。
- **Outputs**：`smell-report`、`overall-score`、`dimension-scores`、`review-input`。
- **Quality Gate**：供下游消费前必须通过 `## Quality Gates` 的可复现性检查。
...
开始时明确说明：  
“我正在使用 `arc:score`，先扫描质量信号，再输出可复用量化评分。”

## The Iron Law

```
NO QUALITY SCORE WITHOUT REPRODUCIBLE DETECTION EVIDENCE
```
...
1. 建立项目快照并识别语言与检测范围。
2. 执行 smell / bugfix / architecture 三类检测。
3. 聚合维度分数并生成评分卡与交接输入。
4. 发布可供 `arc:review` 与 `arc:gate` 复用的产物。

## Quality Gates

- 每项违规必须包含规则 ID、严重度与定位信息。
- 评分计算公式必须可追溯且稳定。
- 产物目录结构必须完整且可被下游消费。
- 检测失败或数据缺失必须显式中断并说明原因。
...
- **首选触发**：需要量化质量分、smell 清单与机器可读评估产物。
- **典型场景**：为 `arc:review` 与 `arc:gate` 提供统一输入。
- **边界提示**：策略级阻断判定用 `arc:gate`，治理路线设计用 `arc:review`。
