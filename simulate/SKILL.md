...
开始时明确说明：  
“我正在使用 `arc:e2e`，将按真实用户路径执行 E2E 并沉淀可审计证据。”
...
4. 输出 PASS/FAIL 证据，必要时回流 `arc:fix`。
...
1. 优先检索以下产物：
   ...
   * `arc:audit` 快照/诊断报告（已知风险点）
   * score 输出（由 `score/` 模块生成，高风险维度）
   * `arc:build` handoff（本轮改动范围）
2. 验证产物新鲜度：`expires_at` + `content_hash`。
3. 产物可用则直接加载，不重复做全量扫描。
4. 产物失效则按 `refresh_skill` 回流更新（`arc:init:update` / `arc:cartography` / `score` 模块刷新（由 `arc:release` 编排触发）/ `arc:audit`）。
...
4. **回流更新建议**：
   ...
   - 评分/评审产物问题 → `score` 模块刷新（由 `arc:release` 编排触发）/ `arc:audit` 更新
