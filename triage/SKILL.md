---
name: arc:fix
description: "arc:e2e 失败或线上故障时使用：定位根因、推动恢复，并产出 fail-to-pass 证据链。"
---

# UI/UX 缺陷排查与修复（基于 arc:e2e，工业化）

## Overview

将 arc:e2e 失败工件或线上故障线索转化为：最小复现/止损 → 根因定位 → 修复与恢复 → 通过证据。

## Quick Contract

- **Trigger**：`arc:e2e` 出现 FAIL、CI 测试间歇失败，或线上故障需要止损恢复。
...
开始时明确说明：  
“我正在使用 `arc:fix`，先固定失败证据，再定位根因并验证修复。”
...
- **边界提示**：尚无可复现失败工件时先执行 `arc:e2e`；仅做质量评审结论用 `arc:audit`。
...
- 若没有 `run_dir`，则需要 arc:e2e 的核心参数：`test_objective`、`personas`、`target_url`（以及可选 `validation_container`）
...
### 0) 若缺少 run_dir：先跑一轮 arc:e2e 产出失败工件
...
1. 检索可复用产物：`codemap.md`、`arc:audit` 快照、score 结果（由 `score/` 模块生成）、`arc:build` handoff
2. 校验 `expires_at`、`content_hash`、文件存在性
3. 可用则直接加载并缩小排查范围
4. 失效则按 `refresh_skill` 回流触发更新（`arc:init:update` / `arc:cartography` / `arc:audit` / `score` 模块刷新（由 `arc:release` 编排触发））
...
4. 如评审产物不存在或过期：先触发 `arc:audit` 更新后再继续
...
若判断为测试脚本/选择器误报：先修正 arc:e2e 的步骤与选择器，再回归一次，确认真实缺陷是否仍存在。
...
注意：arc:e2e 允许明文记录账号/密码；因此 `reports/` 不要提交到仓库。
## Anti-Patterns
