...
1. 检索可复用产物：`codemap.md`、`arc:review` 快照、score 结果（由 `score/` 模块生成）、`arc:implement` handoff
2. 校验 `expires_at`、`content_hash`、文件存在性
3. 可用则直接加载并缩小排查范围
4. 失效则按 `refresh_skill` 回流触发更新（`arc:init:update` / `arc:cartography` / `arc:review` / `score` 模块刷新（由 `arc:gate` 编排触发））
